...
install.packages("pacman", verboseFALSE, quiet= TRUE)
pacman::p_load(
  caret,
  tidyverse,
  readr,
...
colSums(is.na(concrete_data), na.rmFALSE)
```
...
GA_XGBoost_Linear_model <- caret::train(Strength ~., 
                          ...
                          verboseFALSE, metric = "RMSE", maximize = FALSE,
                          silent = 1,
                          # tuneLength = 1
...
Summary_Table_Training %>% 
  ...
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width= TRUE, position = "center") %>%
...
RMSE <- signif(RMSE(pred = test_set$XGBoost_Linear_GS, obs = test_set$Strength, na.rm= TRUE), 6)
...
RMSE <- signif(RMSE(pred = test_set$XGBoost_Linear_RS, obs = test_set$Strength, na.rm= TRUE), 6)
...
RMSE <- signif(RMSE(pred = test_set$XGBoost_Linear_GA, obs = test_set$Strength, na.rm= TRUE), 6)
...
RMSE <- signif(RMSE(pred = test_set$XGBoost_Linear_DE, obs = test_set$Strength, na.rm= TRUE), 6)
...
RMSE <- signif(RMSE(pred = test_set$XGBoost_Linear_PSO, obs = test_set$Strength, na.rm= TRUE), 6)
